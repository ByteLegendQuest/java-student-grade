...
`git clone https://github.com/ByteLegendQuest/java-student-grade`

A project directory named `java-student-grade` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-student-grade` directory which is created just now.
